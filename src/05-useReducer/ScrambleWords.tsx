...
import { SkipForward, Play, RotateCcw } from "lucide-react";
...
export const ScrambleWords = () => {
  ...
  const handleGuessSubmit = (e: React.SubmitEvent) => {
    ...
    if (guess === currentWord) {
      // Quito la cantidad de palabras del array
      ...
      setGuess("");
      setWords(updateWords); // Actualizamos las palabras descontando del array
      setCurrentWord(updateWords[0]); // Actualizamos la palabra actual con la primera posición del nuevo array
      setScrambledWord(scrambleWord(updateWords[0])); // Desordenamos la palabra de la primera posición del nuevo array
    ...
    // Sumanos un punto de error si no adivina
    ...
    return (
      <div className="min-h-screen bg-linear-to-br from-purple-100 via-blue-50 to-indigo-100 flex items-center justify-center p-4">
        <div className="w-full max-w-md flex flex-col gap-8 items-center">
          <h1 className="text-4xl text-center font-bold bg-linear-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent mb-2">
            Palabras desordenadas
          </h1>
          <p className="text-gray-600 text-xl">No hay palabras para jugar</p>
          ...
          <Button className="py-6 px-8 text-lg" onClick={handlePlayAgain}>
